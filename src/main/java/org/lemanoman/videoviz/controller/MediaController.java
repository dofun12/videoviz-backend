...
public class MediaController {
    ...
    @Value("classpath:unknow.png")
    Resource resourceFile;


    // supports byte-range requests
    @GetMapping("/old/video/{filename}")
    public void getVideoOld(HttpServletRequest request, HttpServletResponse response, @PathVariable String filename)
            throws ServletException, IOException {
        File mp4File = new File(mp4folder + File.separator + filename + ".mp4");
        if (mp4File.exists()) {
            request.setAttribute(MyResourceHttpRequestHandler.ATTR_FILE, mp4File);
        }
        handler.handleRequest(request, response);
    }

    /**
     * Copyed from BalusC post, just adapted to springboot
     * Original post:
    ...
    public void getImage(HttpServletRequest request, HttpServletResponse response, @PathVariable String filename)
            throws ServletException, IOException {
        File imgFile = new File(imageLocation + File.separator + filename + ".png");
        if (imgFile.exists()) {
            request.setAttribute(MyResourceHttpRequestHandler.ATTR_FILE, imgFile);
        }else{
            request.setAttribute(MyResourceHttpRequestHandler.ATTR_FILE, resourceFile.getFile());
        }
        handler.handleRequest(request, response);
    }**/
